...
def clean_data(df):
    ...
    for col in df.columns:
        if df[col].dtype == 'object':
            try:
                # Coba bersihkan format ribuan '.' dan desimal ',' (Format Indo)
                ...
                # Jika mayoritas data valid (bukan NaN setelah convert), gunakan hasil convert
                # Kita cek jika setidaknya 50% data berhasil dikonversi
                if converted_col.notna().mean() > 0.5:
                    df[col] = converted_col
            except:
                continue
                
        # Coba convert kolom object yang mungkin angka standar tapi terdeteksi object
        if df[col].dtype == 'object':
            try:
