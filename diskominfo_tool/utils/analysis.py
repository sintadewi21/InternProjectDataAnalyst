...
def check_homoscedasticity(residuals, X):
    ...
    """
    try:
        # Breusch-Pagan butuh X dengan konstanta
        # X harus array like
        ...
        # test_result: lm_stat, lm_pvalue, fvalue, f_pvalue
        return {
...
def perform_forecasting(df, time_col, target_col, periods=5):
    ...
    try:
        ...
        # 1. Coba deteksi apakah time_col adalah Tanggal
        try:
            # Coba convert ke datetime
            # dayfirst=True penting untuk format ID/EU (DD/MM/YYYY)
            data['temp_date'] = pd.to_datetime(data[time_col], dayfirst=True, errors='coerce')
            
            # Jika > 80% berhasil dicast ke date, maka ini adalah kolom tanggal
            if data['temp_date'].notna().mean() > 0.8:
                is_date = True
                data = data.dropna(subset=['temp_date'])
                data = data.sort_values(by='temp_date')
                data[time_col] = data['temp_date'] # Override kolom asli dengan datetime object
                
                # Coba infer frekuensi (misal: 'D', 'M', 'Y') untuk future range
                if len(data) > 3:
                     # Ambil 3 data terakhir untuk menebak delta
                     diffs = data[time_col].diff().dropna()
                     min_diff = diffs.min()
                     # Simple heuristic
                     if min_diff >= pd.Timedelta(days=360): freq = 'Y' # Tahunan
                     elif min_diff >= pd.Timedelta(days=28): freq = 'M' # Bulanan
                     else: freq = 'D' # Harian
            else:
                # Jika bukan tanggal, coba numerik (Tahun)
        ...
        # Generate Future X-Axis
        if is_date:
            last_date = data[time_col].max()
            # Default ke 'D' jika freq gagal dideteksi
            use_freq = freq if freq else 'D'
            
            # Buat range tanggal ke depan
            # periods+1 karena start inclusive, kita butuh next step
            future_dates = pd.date_range(start=last_date, periods=periods+1, freq=use_freq)[1:]
            
            # Jika generate gagal (misal date range error), fallback ke integer step
        ...
        # Hitung MSE pada data historis (Fitted values vs Aktual)
...
def perform_backpropagation_forecasting(df, time_col, target_col, periods=5):
    ...
    try:
        ...
        # Konversi time_col ke numerik jika bukan tanggal
        try:
            # Cegah integer diinterpretasi sebagai nanoseconds by converting to string first for detection
            sample_str = data[time_col].astype(str)
            temp_date = pd.to_datetime(sample_str, dayfirst=True, errors='coerce')
            
            # Jika mayoritas berhasil dan bukan sekadar angka tahun (4 digit)
            # atau jika memang formatnya tanggal (ada pemisah seperti / atau -)
            is_likely_date = temp_date.notna().mean() > 0.8
            
            # Check if it's just year integers
            if is_likely_date:
                # Jika semua bisa jadi int dan length == 4, mungkin itu tahun
                try:
        ...
        if len(data) < 4: # Butuh minimal data yang cukup untuk lag
            return None
            
        # 2. Persiapan Fitur (Lagging)
        # Kita gunakan 3 lag terakhir sebagai fitur untuk memprediksi nilai berikutnya
        y = data[target_col].values
        X_lag = []
        y_lag = []
        ...
        # Arsitektur sederhana: 1 hidden layer dengan 50 neuron
        model = MLPRegressor(
        ...
        for _ in range(periods):
            ...
            # Update batch untuk shift window
...
def perform_multiple_regression(df, x_cols, y_col):
    ...
    return {
        ...
        'mse': mse,
        'X': X,
        'X_with_const': X_with_const, # Needed for tests
        'y_actual': y,
        'y_pred': y_pred,
        'residuals': residuals,
        'f_value': model.fvalue,
...
def get_descriptive_stats(df):
    ...
    # Menggunakan describe() untuk basic stats
    stats = numeric_df.describe().T
    
    # Menghapus quartiles yang tidak diminta (25%, 50%, 75%)
    # 50% adalah median, tapi kita akan buat kolom eksplisit 'Median' agar lebih jelas
    stats = stats.drop(columns=['25%', '50%', '75%'], errors='ignore')
    
    # Menambahkan Median dan Modus
    stats['Median'] = numeric_df.median()
    
    # Modus bisa multiple, kita ambil yang pertama saja untuk ringkasan
    # atau digabung jika perlu. Untuk simplisitas tabel, ambil yang pertama.
    stats['Modus'] = numeric_df.mode().iloc[0]
    
    # Mengatur urutan kolom agar rapi
    target_columns = ['count', 'mean', 'std', 'min', 'Median', 'Modus', 'max']
    # Filter hanya kolom yang ada (jaga-jaga jika ada versi pandas beda output describe)
    final_cols = [col for col in target_columns if col in stats.columns]
    
    stats = stats[final_cols]
    
    # Rename kolom agar lebih Indonesia
